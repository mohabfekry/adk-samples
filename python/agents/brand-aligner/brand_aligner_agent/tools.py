...
async def _upload_part_to_gcs(
  ...
  try:
    file_name = (
        filename or part.inline_data.display_name
...
async def _upload_artifact_to_gcs(
    user_id: str, filename: str, file_name_suffix: str,
    tool_context: ToolContext
) -> None:
...
async def _get_processed_guideline(
    gcs_uri: str,
) -> Tuple[Optional[Guideline], Optional[Blob]]:
  """
...
async def _process_single_guideline(
  ...
  if guideline:
    ...
    logger.info(
        "Processing guideline file: %s with MIME type: %s", gcs_uri, mime_type
    )
...
async def _evaluate_single_asset(
    ...
    asset_id: str,
    category: str,
    asset_prompt: Optional[str] = "",
    video_reference_image_uris: Optional[List[str]] = [],
) -> AssetEvaluation:
  ...
  for gv in asset_evaluation.guideline_verdicts:
    ...
    for verdict in gv.verdicts:
      ...
      c_display = (
          ...
          if verdict.criterion_name else verdict.criterion_id
      )
...
async def asset_evaluator_tool(
  ...
  for evaluation in results:
    ...
    report = (
        f"### Evaluation Result for Asset: {evaluation.asset_id}\n{formatted_table}"
    )
