...
class GuidelineService:
  ...
  async def extract_guideline_from_doc(
    ...
    try:
      ...
      raise Exception(f"Failed to process document with AI: {str(e)}") from e
  ...
  try:
    ...
    for qa in data.get("qas", []):
      record = CriterionRubric(
          ...
          gt_justification=qa.get("justification", ""),
...
def _compute_scores_from_result(df: pd.DataFrame, guideline: Guideline | None):
  ...
  severity_map = ({
      c.criterion_id: c.severity
      for c in guideline.criteria
  } if guideline else {})
  criterion_name_map = ({
      c.criterion_id: c.name
      for c in guideline.criteria
  } if guideline else {})
  criterion_category_map = ({
      c.criterion_id: c.category
      for c in guideline.criteria
  } if guideline else {})
  ...
  for _, row in df.iterrows():
    ...
    for rubric in rubrics:
      ...
      justification = verdict_dict.get(
          "justification", rubric.gt_justification
      ...
      criterion_verdicts.append(
          CriterionVerdict(
              ...
              gt_answer=gt_answer,
              gt_justification=rubric.gt_justification,
              verdict=verdict,
              justification=justification,
          )
      )
...
class EvalService:
  ...
  def _get_asset_type(self,
                      asset_uri: str) -> Literal["video", "image", "unknown"]:
  ...
  def evaluate_asset(
    ...
    guideline_id = (
        ...
        if valid_prompt else "gqm_quality"
    )
    ...
    for guideline in guidelines:
      ...
      relevant_criteria = (
          ...
          if len(guideline.criteria) > self.batch_size else guideline.criteria
      )
  ...
  def _run_batch_evaluation(
    ...
    batch_size = (
        ...
        if "rubrics_internal" in batch_dataset.columns else "unknown"
    )
  ...
  def _get_or_generate_rubrics(
    ...
    if use_cache:
      ...
      content_str = (
          f"{guideline_id}|{prompt_template}|{json.dumps(prompt_params, sort_keys=True)}"
      )
  ...
  def _execute_evaluation(
    ...
    eval_dataset["rubrics_internal"] = None
    eval_dataset["rubrics_internal"] = eval_dataset["rubrics_internal"].astype(
        object
    )
      ...
      for row_index in metrics_table.index:
        new_rubrics = [
            CriterionRubric(
                ...
                gt_justification=row.gt_justification,
...
def _get_mime_type(file_path: str) -> str:
  ...
  file_name = f if "." in f else str(p.parents[0]
                                    ).replace(f"{str(p.parents[1])}/", "")
