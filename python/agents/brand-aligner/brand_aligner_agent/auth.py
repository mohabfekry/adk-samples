...
def _get_token_from_state(tool_context: ToolContext, key: str) -> Optional[str]:
...
def get_user_id(tool_context: ToolContext) -> Union[str, Dict[str, str]]:
  """Retrieves the user's id to use in the agent's operations."""
  # Gemini Enterprise logic - auth flow already finished!
  token = _get_token_from_state(tool_context, AUTH_ID) or _get_token_from_state(
      tool_context, f"temp:{AUTH_ID}"
  )
  ...
  try:
    if cached_token_info:
      ...
      try:
        ...
        logger.info(f"AUTH: Error loading/refreshing cached creds: {e}")
        creds = None
        tool_context.state[TOKEN_CACHE_KEY] = None
